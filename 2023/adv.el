...
;; Day 12
(defun adv/day12-parse-line (line)
  "Parse LINE."
  (let* ((sp (s-split " " line))
         (springs (seq-into (car sp) 'list))
         (nums (-map #'adv/num (s-split "," (cadr sp)))))
    (list nil springs nums)))
(defvar adv/day12-expand-cache (ht-create))
(defun adv/day12-expand (cur)
  "Expand CUR recursively."
  (if-let ((cached (ht-get adv/day12-expand-cache cur)))
      cached
    (let* ((last (car cur))
           (up (cadr cur))
           (c (car up))
           (groups (caddr cur))
           (g (car groups))
           (rembroken (-sum groups))
           (future (length up))
           (futurebroken (--count (= it ?#) up))
           (futureunknown (--count (= it ??) up))
           (ret (cond
                 ((< future (-sum groups))
                  0)
                 ((> futurebroken (-sum groups))
                  0)
                 ((< (+ futurebroken futureunknown) rembroken)
                  0)
                 ((and (not groups) (not up))
                  1)
                 ((and (not c) last (= last ?#) (= g 0))
                  (adv/day12-expand (list last up (cdr groups))))
                 ((and c (= c ?.) last (= last ?#) (= g 0))
                  (adv/day12-expand (list c (cdr up) (cdr groups))))
                 ((and c (= c ?.) last (= last ?#))
                  0)
                 ((and c (= c ?.))
                  (adv/day12-expand (list c (cdr up) groups)))
                 ((and c (= c ?#) g (> g 0))
                  (adv/day12-expand (list c (cdr up) (cons (- g 1) (cdr groups)))))
                 ((and c (= c ??))
                  (+
                   (adv/day12-expand (list last (cons ?. (cdr up)) groups))
                   (adv/day12-expand (list last (cons ?# (cdr up)) groups))))
                 (t 0))))
      (ht-set! adv/day12-expand-cache cur ret)
      ret)))
(defun adv/day12-part1 ()
  "Solve day 12 part 1."
  (let* ((lines (adv/lines "12/input.txt"))
         (rows (-map #'adv/day12-parse-line lines))
         (counts (--map (adv/day12-expand it) rows))
         )
    (-sum counts)
    ))
(defun adv/day12-parse-line-5x (line)
  "Parse LINE and repeat 5 times."
  (let* ((sp (s-split " " line))
         (springs (cdr (adv/cat (-repeat 5 (cons ?? (seq-into (car sp) 'list))))))
         (nums (adv/cat (-repeat 5 (-map #'adv/num (s-split "," (cadr sp)))))))
    (list nil springs nums)))
(defun adv/day12-part2 ()
  "Solve day 12 part 2."
  (let* ((lines (adv/lines "12/input.txt"))
         (rows (-map #'adv/day12-parse-line-5x lines))
         (counts
          (--map-indexed
           (let ((res (adv/day12-expand it)))
             (message "%s. %s" it-index res)
             res)
           rows))
         )
    (-sum counts)
